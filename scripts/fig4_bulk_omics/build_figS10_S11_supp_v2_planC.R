...
# Builds: Supplementary Figs. S17, S18 - stromal and lineage panels for Fap-deficient hosts.
...
heatmap_delta <- function(df, title_str, tag_str, subtitle_str = NULL){
  ...
  ggplot(dset, aes(dataset, signature, fill = delta_fap_minus_wt)) +
    geom_tile(colour = "white", linewidth = 0.6) +                 ## v2 Plan C: thicker tile separators (0.3 → 0.6)
    geom_text(aes(label = label_disp,
                  colour = abs(delta_fap_minus_wt) > max_abs * 0.6),
              size = (base_font_size + 0.0)/.pt,                   ## v2 Plan C: ~7pt (was 5.5pt)
              fontface = "bold",                                    ## v2 Plan C: bold for readability
              show.legend = FALSE) +
    ...
    theme(axis.line = element_blank(), axis.ticks = element_blank(),
          axis.text.y = element_text(size = base_font_size),       ## v2 Plan C: 7pt (was 6.5pt)
...
## The old title "Fig. S10. ..." also referenced stale pre-2026-05-12 numbering.
fig_s10 <- s10_a + s10_b + s10_c + s10_d +
  plot_layout(design = s10_design) &
  theme(plot.tag.position = c(0.01, 1.02))

ggsave(file.path(fig_dir, "FigS20_v2_planC_standardized_signatures.pdf"),
       fig_s10, width = 260, height = 240, units = "mm",
       device = grDevices::pdf, useDingbats = FALSE)
ggsave(file.path(fig_dir, "FigS20_v2_planC_standardized_signatures.png"),
       fig_s10, width = 260, height = 240, units = "mm",
...
s11_e <- ggplot(box_df, aes(group, fraction, fill = group)) +
  geom_boxplot(width = 0.55, outlier.shape = NA,
               linewidth = 0.3, alpha = 0.85) +
  geom_jitter(width = 0.12, height = 0, size = 0.9,                  ## v2 Plan C: jitter dot larger (0.7→0.9)
              shape = 21, stroke = 0.2, colour = "black") +
  geom_text(data = box_stat, aes(x = 1.5, y = Inf, label = label),
            inherit.aes = FALSE, vjust = 1.3,
            size = (base_font_size - 0.3)/.pt) +                     ## v2 Plan C: sig star ~6.7pt (was 5.2pt)
  ...
  labs(x = NULL, y = "Fraction",
       ...
       tag = "e") +
  theme_nc() +
  theme(strip.text = element_text(size = base_font_size,             ## v2 Plan C: 7pt (was 6pt)
                                  face = "bold"),                    ## v2 Plan C: bold strip
        panel.spacing.x = unit(3, "mm"))                             ## v2 Plan C: more white space (1.5→3mm)
...
s11_g <- ggplot(ulm_df %>%
  ...
  geom_boxplot(width = 0.55, outlier.shape = NA,
               linewidth = 0.3, alpha = 0.85) +
  geom_jitter(width = 0.12, height = 0, size = 0.9,                  ## v2 Plan C: jitter dot larger
              shape = 21, stroke = 0.2, colour = "black") +
  geom_text(data = ulm_stat, aes(x = 1.5, y = Inf, label = label),
            inherit.aes = FALSE, vjust = 1.3,
            size = (base_font_size - 0.3)/.pt) +                     ## v2 Plan C: sig star ~6.7pt
  facet_wrap(~ celltype, nrow = 1, scales = "free_y") +
  scale_fill_manual(values = grp_cols, guide = "none") +
  scale_x_discrete(labels = c("WT host" = "WT",
                              "FAP-deficient host" = "KO")) +
  ...
  labs(x = NULL, y = "ULM activity (z)",
       ...
       tag = "g") +
  theme_nc() +
  theme(strip.text = element_text(size = base_font_size,             ## v2 Plan C: 7pt
                                  face = "bold"),                    ## v2 Plan C: bold strip
        panel.spacing.x = unit(3, "mm"))                             ## v2 Plan C: more white space
...
## In-figure plot title + subtitle stripped per scripts-122/123 convention
## (Supplementary figure title lives in the SI docx caption, not on the figure).
## The old title "Fig. S11. ..." also referenced stale pre-2026-05-12 numbering.
fig_s11 <- s11_a + s11_b + s11_c + s11_d + s11_e + s11_f + s11_g +
  plot_layout(design = s11_design) &
  theme(plot.tag.position = c(0.01, 1.02))

ggsave(file.path(fig_dir, "FigS21_v2_planC_mechanism_refinement.pdf"),
       fig_s11, width = 230, height = 310, units = "mm",
       device = grDevices::pdf, useDingbats = FALSE)
ggsave(file.path(fig_dir, "FigS21_v2_planC_mechanism_refinement.png"),
       fig_s11, width = 230, height = 310, units = "mm",
       dpi = 600, device = ragg::agg_png)
